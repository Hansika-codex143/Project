    ...
    def perform_analysis(self):
        ...
        try:
            ...
            if self.ocr_var.get() or not input_file.lower().endswith(('.txt', '.doc', '.docx')):
                ...
                if input_file.lower().endswith(('.jpg', '.jpeg', '.png', '.bmp', '.tiff')):
                 # For image files
                 image = Image.open(input_file)
                 text_content = pytesseract.image_to_string(image)
                elif input_file.lower().endswith('.pdf'):
                  # For PDFs you'll need to extract images from pages
                  # This is a simplified version - you might need a PDF library like PyPDF2 or pdf2image
                  print("PDF processing requires additional libraries")
                  text_content = "PDF text extraction not implemented in this version"
                else:
